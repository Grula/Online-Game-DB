...
DELIMITER $$
USE `game`$$
CREATE DEFINER = CURRENT_USER TRIGGER `game`.`level_AFTER_INSERT` AFTER INSERT ON `level` FOR EACH ROW
BEGIN
	insert into level_has_oponent 
    set level_id = new.id,
		oponent_id = ( Select FLOOR(RAND()*(MAX(id)+1)) from oponent); -- random
END$$
